...
export default function SiteHeader({
  ...
  const processedCategories = allCategories.slice(0, 6).map((category) => ({
    ...
    description: category.description || `View our ${category.title} collection`,
    image: category.imageUrl || "/placeholder-image.jpg",
    href: `/gallery`,
  }))

  // Combine base menu items with processed categories
  const menuItems = baseMenuItems.map((item) => (item.hasChildren ? { ...item, children: processedCategories } : item))
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {(session.user?.role === "admin" || session.user?.role === "service_provider") && (
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-b from-amber-50 to-white border-b border-amber-100">
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
                    ...
                    <>
                      ...
                      <NavigationMenuContent>
                        <div className="w-[400px] p-4 md:w-[500px] lg:w-[600px] bg-white">
                          <h3 className="text-lg font-semibold mb-2 text-amber-700">{item.title}</h3>
        ...
        <div className="hidden lg:flex items-center space-x-4">
          <Button variant="outline" size="sm" className="border-amber-200 hover:text-amber-700">
            <Link
              href="https://youtube.com/@sallymediaug?si=6n4WdgWgoC6oWJ-E"
              target="_blank"
              className="flex items-center gap-2"
            >
              <Image width={1080} height={1080} src="/youtube.png" className="w-6 h-4" alt="YouTube" />
              Youtube
            </Link>
          </Button>
          <AuthButton />
          ...
          <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
            ...
            >
              ...
              <nav className="flex flex-col space-y-6 mt-6">
                {menuItems.map((item) => (
                  <div key={item.title} className="border-b border-amber-100 pb-4">
                    {!item.hasChildren ? (
                      <button
                      ...
                      <>
                        <h3 className="text-lg font-semibold mb-3 text-amber-700">{item.title}</h3>
