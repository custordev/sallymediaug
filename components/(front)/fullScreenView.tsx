  ...
  return (
    ...
    >
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute top-4 right-4  hover:bg-amber-500 hover:text-black border-white/20 z-50 transition-all duration-200"
        onClick={memoizedCloseFullscreen}
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute left-4 top-1/2 -translate-y-1/2  hover:bg-amber-500 hover:text-black border-white/20 z-50 transition-all duration-200"
        onClick={(e) => {
          e.stopPropagation();
          memoizedPrevImage(e);
        }}
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute  right-4 top-1/2 -translate-y-1/2  hover:bg-amber-500 hover:text-black border-white/20 z-50 transition-all duration-200"
        onClick={(e) => {
          e.stopPropagation();
          memoizedNextImage(e);
        }}
      ...
      <div className="absolute bottom-4 flex items-center gap-3 bg-black/50 backdrop-blur-sm rounded-full px-4 py-2">
        ...
        >
          <PlayCircle
            className="w-5 h-5"
            fill={isSlideshow ? "currentColor" : "none"}
          />
        </Button>
      </div>
      ...
      {isSlideshow && (
        <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 bg-black/70 backdrop-blur-sm px-4 py-2 rounded-full flex items-center gap-4">
          ...
          <span className="text-amber-400 font-medium">
            {formatTime(slideshowDelay / 1000)}
          </span>
          <button
            onClick={(e) => {
              e.stopPropagation();
